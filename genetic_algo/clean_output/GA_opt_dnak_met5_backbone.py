...
def compute_fitness(location_file,peptides,nround):
    ...
    new_pep=[item for item, count in zip(peptides, rep) for i in range(count)]
    new_index=[item for item, count in zip(index, rep) for i in range(count)]
    Interaction_temp=[]
    for j in range(len(new_pep)):
        temp_count=new_pep[j].count("M")
        if temp_count==0:
        	Interaction_temp.append(data.Interaction_Energy[j])
        else:
        	Interaction_temp.append(data.Interaction_Energy[j]+5)
 
    df_temp=pd.DataFrame(list(zip(list(new_pep),Interaction_temp,new_index)),columns=['peptideseq','Interaction_Energy','split'])
    AvgIE = df_temp.groupby('split')['Interaction_Energy'].transform('mean')
    temp_avg=[AvgIE[i] for i in range(0, len(data)-2, 3)]
...
def remove_file():
    print("removing")
    shutil.rmtree("/switchlab/group/krinad/incorporate_structures_dnak/GA_round0/") 
    subprocess.run("mkdir /switchlab/group/krinad/incorporate_structures_dnak/GA_round0", shell=True)
...
def get_interaction_energy_mutate(peptide):
    ...
    generate_single_FOLDX_mutate_runfiles("1dkz_Repair_1_0.pdb",0)
    generate_single_FOLDX_analyse_runfiles("1dkz_Repair_1_0.pdb",0)


    path_to_file="/switchlab/group/krinad/incorporate_structures_dnak/GA_round0"
    temop_mq="qsub -N mutate " + path_to_file+"/FOLDX_mutate_round0_pdb"+"_original"+"_dnak.q"
    subprocess.run(temop_mq, shell=True)

    while not os.path.exists("/switchlab/group/krinad/incorporate_structures_dnak/GA_round0/1dkz_Repair_1_0_1_2.pdb"):
    	time.sleep(1)

    os.chdir(path_to_file)
    temop_aq="qsub -hold_jid mutate -N analyse " +"FOLDX_analyse_round0_pdb_original_dnak.q"
    subprocess.run(temop_aq, shell=True)   

    while not os.path.exists("/switchlab/group/krinad/incorporate_structures_dnak/GA_round0/Summary_1dkz_Repair_1_0_1_2_AC.fxout"):
        time.sleep(1)

    os.chdir("/switchlab/group/krinad/incorporate_structures_dnak/GA_round0")
    command="tail -n1 -q Summary_1dkz_Repair_1_0_1_* >> Summary_file.txt"
    subprocess.run(command, shell=True)

    time.sleep(10)

    temp_ie=pd.read_csv("/switchlab/group/krinad/incorporate_structures_dnak/GA_round0/Summary_file.txt", delimiter="\s+")
    ...
    os.chdir("/switchlab/group/krinad/incorporate_structures_dnak/")
...
def generate_single_individual_list(peptide):
    with open('/switchlab/group/krinad/incorporate_structures_dnak/GA_round0/individual_list.txt','w',newline='\n') as myfile:
        temp_line=[]
        count=1
        for i in peptide:
            if (count==7):
                temp_line.append("A"+"B"+str(count)+i+";\n")
            else:
                    temp_line.append("A"+"B"+str(count)+i+",")
                    count=count+1
        myfile.write("".join(temp_line))
...
def generate_single_FOLDX_mutate_runfiles(originalPDB,nround):
    temp_file="/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/config_mutate_round"+str(nround)+"_pdb"+"_original"+"_dnak.cfg"
    with open(temp_file,'w',newline='\n') as configfile:
                text_list_config = ["command=BuildModel\n","pdb="+originalPDB+"\n",
                    "pdb-dir=/switchlab/group/krinad/dnak\n",
                    #"rotabaseLocation/switchlab/group/krinad/incorporate_structures_dnak/rotabase.txt\n",
                    "mutant-file=/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/individual_list.txt\n",
                    "output-dir=/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"\n",
                    "numberOfRuns=3\n"]
    ...
    temp_file_q="/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/FOLDX_mutate_round"+str(nround)+"_pdb"+"_original"+"_dnak.q"
    with open(temp_file_q,'w',newline='\n') as qfile:
        text_list_q = ["#!/bin/bash\n",
                            "#$ -N FOLDX_1dkz_round"+str(nround)+"_hc\n",
                            "#$ -cwd\n",
                            "#$ -V\n" ,
                            "#$ -q all.q\n" ,
                            "#$ -l h_vmem=512M\n",
                            "\n",
                            "/switchlab/group/tools/2021_FoldX5_LoopX/foldx -f /switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/config_mutate_round"+str(nround)+"_pdb"+"_original"+"_dnak.cfg"+"\n"]
...
def generate_single_FOLDX_analyse_runfiles(originalPDB, nround):
    temp_file="/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/config_analyse_round"+str(nround)+"_pdb_"+"original"+"_dnak.cfg"
    with open(temp_file,'w',newline='\n') as configfile:
        text_list_config = ["command=AnalyseComplex\n","pdb-list=/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/pdb-list.txt\n",
            "analyseComplexChains=A,B\n",
            "output-dir=/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"\n"]
        configfile.writelines(text_list_config)
        configfile.close()

    temp_file_q="/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/FOLDX_analyse_round"+str(nround)+"_pdb_"+"original"+"_dnak.q"
    with open(temp_file_q,'w',newline='\n') as qfile:
        text_list_q = ["#!/bin/bash\n",
                        "#$ -N FOLDX_1dkz_round"+str(nround)+"_ana_hc\n",
                        "#$ -cwd\n",
                        "#$ -V\n" ,
                        "#$ -q all.q\n" ,
                        "#$ -l h_vmem=512M\n",
                        "\n",
                        "/switchlab/group/tools/2021_FoldX5_LoopX/foldx -f /switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/config_analyse_round"+str(nround)+"_pdb_"+"original"+"_dnak.cfg\n"]
        qfile.writelines(text_list_q)
        qfile.close()
        
    temp_file="/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/pdb-list.txt"
    with open(temp_file,'w',newline='\n') as pdbfile:
        for i in range(0,3):
            pdbfile.write("/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/"+"1dkz_Repair_1_0_1_"+str(i)+".pdb;\n")
...
def get_interaction_energy_full(peptides,count):
    create_indlist(peptides,"B",1,7,count)
    generate_FOLDX_mutate_runfiles("1dkz_Repair_1_0.pdb",count)
    generate_FOLDX_analyse_runfiles("1dkz_Repair_1_0.pdb",count)
    run_files(count)
    location_file="/switchlab/group/krinad/incorporate_structures_dnak/"+"Summary"+str(count)+"_file.txt"
...
def generate_FOLDX_mutate_runfiles(originalPDB,nround):
    temp_analyse="/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/analyse/Summary"+str(nround)+"_file.txt"
    ...
    for i in range(1,101):
        ...
        completeName = os.path.join(path_to_file, "config_mutate_round"+str(nround)+"_pdb"+"_original"+"_dnak.cfg")
        temp_file="./GA_round"+str(nround)+"/config_mutate_round"+str(nround)+"_R"+str(i)+"_pdb"+"_original"+"_dnak.cfg"
        with open(completeName,'w') as configfile:
            text_list_config = ["command=BuildModel\n","pdb="+originalPDB+"\n",
                "pdb-dir=/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/R"+str(i)+"\n",
                #"rotabaseLocation=/switchlab/group/krinad/GA_round"+str(nround)+"/R"+str(i)+"/rotabase.txt\n",
                "mutant-file=/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/R"+str(i)+"/individual_list.txt\n",
                "output-dir=/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/R"+str(i)+"\n",
                "numberOfRuns=3\n"]
            configfile.writelines(text_list_config)
        configfile.close()
        ...
        path_to_file="./GA_round"+str(nround)+"/R"+str(i)
        completeName = os.path.join(path_to_file, "FOLDX_mutate_round"+str(nround)+"_R"+str(i)+"_pdb"+"_original"+"_dnak.q")
        temp_list_summary=(["mv /switchlab/group/krinad/Summary_file.txt /switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/analyse\n"]+["\n"] * 99)
        ...
        with open(completeName,'w',newline='\n') as qfile:
            text_list_q = ["#!/bin/bash\n",
                                ...
                                "#$ -l h_vmem=512M\n",
                                "\n",
                                "cp /switchlab/group/krinad/incorporate_structures_dnak/"+originalPDB+" /switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/R"+str(i)+"\n",
                                #"cp /home/krinad/thesisfiles/dnak/rotabase.txt /switchlab/group/krinad/GA_round"+str(nround)+"/R"+str(i)+"\n",
                                "/switchlab/group/tools/2021_FoldX5_LoopX/foldx -f /switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/R"+str(i)+"/config_mutate_round"+str(nround)+"_pdb"+"_original"+"_dnak.cfg"+"\n"]
...
def generate_FOLDX_analyse_runfiles(originalPDB, nround):
        completeName = os.path.join("/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround), "pdb-list.txt")
        #temp_file="/home/krinad/thesisfiles/dnak/buildmodel_AlanineMutagenesis/GA_trial/GA_round"+str(nround)+"/pdb-list.txt"
        with open(completeName,'w',newline='\n') as pdbfile:
            for i in range(1,101):
                for j in range(0,3):
                    pdbfile.write("/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/1dkz_Repair_1_0_"+str(i)+"_"+str(j)+".pdb;\n")
        pdbfile.close()

        path_to_file="./GA_round"+str(nround)
        completeName = os.path.join(path_to_file, "config_analyse_round"+str(nround)+"_pdb_"+"original"+"_dnak.cfg")
        #temp_file="./GA_round"+str(nround)+"config_analyse_round"+str(nround)+"_pdb_"+"original"+"_dnak.cfg"
        with open(completeName,'w',newline='\n') as configfile:
            text_list_config = ["command=AnalyseComplex\n","pdb-list=/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/pdb-list.txt\n",
                "analyseComplexChains=A,B\n",
                "output-dir=/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/analyse\n"]
            configfile.writelines(text_list_config)
        configfile.close()
    
        completeName1 = os.path.join(path_to_file, "FOLDX_analyse_round"+str(nround)+"_pdb_"+"original"+"_dnak.q")
...
def run_files(nround):
    for i in range(1,101):
        path_to_file="/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/R"+str(i)
        os.chdir(path_to_file)
        temop_mq="qsub -N mutate"+str(i)+" " + path_to_file+"/FOLDX_mutate_round"+str(nround)+"_R"+str(i)+"_pdb"+"_original"+"_dnak.q"
        subprocess.run(temop_mq, shell=True)

    filelist=["/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/R" + str(n)+"/1dkz_Repair_1_0_1_"+str(k)+".pdb" for n in range(1,101) for k in range(0,3)]
    k=True
    while k:
    ...
    for i in range(0,100):
        ...
        for j in chunks[i]:
            for k in range(0,3):
                location="/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/R"+str(i+1)+"/1dkz_Repair_1_0_1_"+str(k)+".pdb"
                destination="/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/1dkz_Repair_1_0_"+str(j)+"_"+str(k)+".pdb"
                command='mv '+location+" "+destination
                subprocess.run(command, shell=True)
                count=count+1  

    path_to_file="/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)        
    os.chdir(path_to_file)
    temop_aq="qsub -N analyse " +"FOLDX_analyse_round"+str(nround)+"_pdb"+"_original"+"_dnak.q"
    subprocess.run(temop_aq, shell=True)   

    os.chdir("/switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/analyse")

    while not os.path.exists("Summary_1dkz_Repair_1_0_100_2_AC.fxout"):
        time.sleep(1)
    command="tail -n1 -q Summary_1dkz_Repair_1_0_*.fxout >> Summary"+str(nround)+"_file.txt"
    subprocess.run(command, shell=True)

    

    command_move="mv /switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/analyse/Summary"+str(nround)+"_file.txt"+ " /switchlab/group/krinad/incorporate_structures_dnak/Summary"+str(nround)+"_file.txt"
    subprocess.run(command_move, shell=True)   

    os.chdir("/switchlab/group/krinad/incorporate_structures_dnak/")

    del_dir="rm -r /switchlab/group/krinad/incorporate_structures_dnak/GA_round"+str(nround)+"/"
    subprocess.run(del_dir, shell=True) 
...
randomlist=['GHLCEWI', 'GPKQAVY', 'AMCTYID', 'SLTGPKH', 'SNGYIRE', 'WMYKEAP', 'SVCQEAP', 'YNFACWG',
 'WYMCPVI', 'ICVFEYD', 'FTDHQYI', 'TVMNDEK', 'SRLCMVP', 'GRWEDYF', 'QFEVGHY', 'QCTLNHD',
 'KDWPLCF', 'VWRDSEC', 'DRIPATV', 'SDKRQEA', 'VDIRWQF', 'ALRDVQY', 'PMGRDWF', 'GDYCKTR',
 'HIDRKES', 'SGMRYLQ', 'TRHIWVE', 'IVCTQPY', 'NVIQSHT', 'GFESHWD', 'YRTGWLD', 'SYIWKGA',
 'PVWDMQL', 'SMDGLEQ', 'WPLEMVK', 'GIRAYQV', 'LGRVIFE', 'WEIMNLA', 'SRPCALK', 'VRHCWED',
 'LSMKCVG', 'SQCVGER', 'YCPKHDE', 'WFHITEA', 'TMPWSNE', 'CKHPMST', 'YRKFWAS', 'CSMIWNQ',
 'CEKWTYH', 'LIMPNVE', 'QMHYCTP', 'ALHTQRK', 'VPYTNRS','NLVPIDH', 'GASNWTL', 'RWHGILY',
 'WSDTHRQ', 'NHCGMTI', 'DIVHTPW', 'EFKGSAT' ,'AQNYRFK', 'IQSYWTG', 'CYLPIGA', 'LPAGEWM',
 'YSTKWFM', 'KARLWIY', 'ICPGDLE', 'VCSGNML', 'RQTAFKN', 'THVRAYQ', 'VRWPIDC',
 'TCKVQWS', 'ELYRTNI', 'APFITCL', 'NGRCWVP', 'NKGMPIV', 'TCNWVSK', 'HLPYICW', 'NPFMSYI',
 'PFLKTYH', 'QVSMFPD', 'FIACPDH', 'YIHDPGK', 'IRLVKNG', 'WCTKVYF', 'CGVNDMA', 'QCIPLNA',
 'YNTKARP', 'SVMHFDT', 'FKNGMVS', 'VDPLFTR', 'QNGITDR', 'TCKINSA', 'GSRDIYW', 'KSYGWHA',
 'AQHTYWF', 'VAQDWET', 'PWINTQM', 'DQMYEKC', 'ELMFYAG']# #create_idlist(randomlist,"B",2,7) # create the individual list    
...
create_indlist(randomlist,"B",1,7,1)
generate_FOLDX_mutate_runfiles("1dkz_Repair_1_0.pdb",1)
generate_FOLDX_analyse_runfiles("1dkz_Repair_1_0.pdb",1)
run_files(1)


data1=compute_fitness("/switchlab/group/krinad/incorporate_structures_dnak/Summary1_file.txt",randomlist,1) 
temp_file_x="compute_fitness1.txt"
with open(temp_file_x, 'w') as f:
    f.write(data1.to_string(header = True, index = False))
f.close()  

median_interact.append(data1.Interaction_Energy.median())
...
temp_file_x="compute_fitness2.txt"
with open(temp_file_x, 'w') as f:
...
median_interact.append(data2.Interaction_Energy.median())

# # # ## CHECK
count=3
#count<11

while (abs(median_interact[count-2])-abs(median_interact[count-3])>0.15):
