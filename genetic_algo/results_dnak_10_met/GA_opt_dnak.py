...
def remove_file():
    print("removing")
    shutil.rmtree("/switchlab/group/krinad/dnak/GA_round0/") 
    subprocess.run("mkdir /switchlab/group/krinad/dnak/GA_round0", shell=True)
...
def get_interaction_energy_mutate(peptide):
    ...
    path_to_file="/switchlab/group/krinad/dnak/GA_round0"
    temop_mq="qsub -N mutate " + path_to_file+"/FOLDX_mutate_round0_pdb"+"_original"+"_dnak.q"
    subprocess.run(temop_mq, shell=True)

    while not os.path.exists("/switchlab/group/krinad/dnak/GA_round0/1dkz_Repair_1_0_1_2.pdb"):
    	time.sleep(1)
    ...
    while not os.path.exists("/switchlab/group/krinad/dnak/GA_round0/Summary_1dkz_Repair_1_0_1_2_AC.fxout"):
        time.sleep(1)

    os.chdir("/switchlab/group/krinad/dnak/GA_round0")
    command="tail -n1 -q Summary_1dkz_Repair_1_0_1_* >> Summary_file.txt"
    subprocess.run(command, shell=True)

    time.sleep(10)

    temp_ie=pd.read_csv("/switchlab/group/krinad/dnak/GA_round0/Summary_file.txt", delimiter="\s+")
    ...
    os.chdir("/switchlab/group/krinad/dnak/")
...
def generate_single_individual_list(peptide):
    with open('/switchlab/group/krinad/dnak/GA_round0/individual_list.txt','w',newline='\n') as myfile:
        temp_line=[]
        count=1
        for i in peptide:
            if (count==7):
...
def generate_single_FOLDX_mutate_runfiles(originalPDB,nround):
    temp_file="/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/config_mutate_round"+str(nround)+"_pdb"+"_original"+"_dnak.cfg"
    with open(temp_file,'w',newline='\n') as configfile:
                text_list_config = ["command=BuildModel\n","pdb="+originalPDB+"\n",
                    ...
                    "mutant-file=/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/individual_list.txt\n",
                    "output-dir=/switchlab/group/krinad/dnak/GA_round"+str(nround)+"\n",
                    "numberOfRuns=3\n"]
    ...
    temp_file_q="/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/FOLDX_mutate_round"+str(nround)+"_pdb"+"_original"+"_dnak.q"
    with open(temp_file_q,'w',newline='\n') as qfile:
        text_list_q = ["#!/bin/bash\n",
                            ...
                            "#$ -l h_vmem=512M\n",
                            "\n",
                            "/switchlab/group/tools/2021_FoldX5_LoopX/foldx -f /switchlab/group/krinad/dnak/GA_round"+str(nround)+"/config_mutate_round"+str(nround)+"_pdb"+"_original"+"_dnak.cfg"+"\n"]
...
def generate_single_FOLDX_analyse_runfiles(originalPDB, nround):
    temp_file="/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/config_analyse_round"+str(nround)+"_pdb_"+"original"+"_dnak.cfg"
    with open(temp_file,'w',newline='\n') as configfile:
        text_list_config = ["command=AnalyseComplex\n","pdb-list=/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/pdb-list.txt\n",
            "analyseComplexChains=A,B\n",
            "output-dir=/switchlab/group/krinad/dnak/GA_round"+str(nround)+"\n"]
        configfile.writelines(text_list_config)
        configfile.close()

    temp_file_q="/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/FOLDX_analyse_round"+str(nround)+"_pdb_"+"original"+"_dnak.q"
    with open(temp_file_q,'w',newline='\n') as qfile:
        text_list_q = ["#!/bin/bash\n",
                        ...
                        "#$ -l h_vmem=512M\n",
                        "\n",
                        "/switchlab/group/tools/2021_FoldX5_LoopX/foldx -f /switchlab/group/krinad/dnak/GA_round"+str(nround)+"/config_analyse_round"+str(nround)+"_pdb_"+"original"+"_dnak.cfg\n"]
        qfile.writelines(text_list_q)
        qfile.close()
        
    temp_file="/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/pdb-list.txt"
    with open(temp_file,'w',newline='\n') as pdbfile:
        for i in range(0,3):
            pdbfile.write("/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/"+"1dkz_Repair_1_0_1_"+str(i)+".pdb;\n")
...
def get_interaction_energy_full(peptides,count):
    ...
    run_files(count)
    location_file="/switchlab/group/krinad/dnak/GA_round"+str(count)+"/analyse/Summary_file.txt"
...
def generate_FOLDX_mutate_runfiles(originalPDB,nround):
    temp_analyse="/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/analyse/Summary_file.txt"
    ...
    for i in range(1,101):
        ...
        with open(completeName,'w') as configfile:
            text_list_config = ["command=BuildModel\n","pdb="+originalPDB+"\n",
                "pdb-dir=/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/R"+str(i)+"\n",
                #"rotabaseLocation=/switchlab/group/krinad/GA_round"+str(nround)+"/R"+str(i)+"/rotabase.txt\n",
                "mutant-file=/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/R"+str(i)+"/individual_list.txt\n",
                "output-dir=/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/R"+str(i)+"\n",
                "numberOfRuns=3\n"]
            configfile.writelines(text_list_config)
        configfile.close()
        ...
        temp_list_summary=(["mv /switchlab/group/krinad/Summary_file.txt /switchlab/group/krinad/dnak/GA_round"+str(nround)+"/analyse\n"]+["\n"] * 99)
        ...
        with open(completeName,'w',newline='\n') as qfile:
            text_list_q = ["#!/bin/bash\n",
                                ...
                                "#$ -l h_vmem=512M\n",
                                "\n",
                                "cp /switchlab/group/krinad/dnak/"+originalPDB+" /switchlab/group/krinad/dnak/GA_round"+str(nround)+"/R"+str(i)+"\n",
                                #"cp /home/krinad/thesisfiles/dnak/rotabase.txt /switchlab/group/krinad/GA_round"+str(nround)+"/R"+str(i)+"\n",
                                "/switchlab/group/tools/2021_FoldX5_LoopX/foldx -f /switchlab/group/krinad/dnak/GA_round"+str(nround)+"/R"+str(i)+"/config_mutate_round"+str(nround)+"_pdb"+"_original"+"_dnak.cfg"+"\n"]
...
def generate_FOLDX_analyse_runfiles(originalPDB, nround):
        completeName = os.path.join("/switchlab/group/krinad/dnak/GA_round"+str(nround), "pdb-list.txt")
        #temp_file="/home/krinad/thesisfiles/dnak/buildmodel_AlanineMutagenesis/GA_trial/GA_round"+str(nround)+"/pdb-list.txt"
        with open(completeName,'w',newline='\n') as pdbfile:
            for i in range(1,101):
                for j in range(0,3):
                    pdbfile.write("/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/1dkz_Repair_1_0_"+str(i)+"_"+str(j)+".pdb;\n")
        pdbfile.close()
        ...
        with open(completeName,'w',newline='\n') as configfile:
            text_list_config = ["command=AnalyseComplex\n","pdb-list=/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/pdb-list.txt\n",
                "analyseComplexChains=A,B\n",
                "output-dir=/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/analyse\n"]
            configfile.writelines(text_list_config)
        configfile.close()
...
def run_files(nround):
    for i in range(1,101):
        path_to_file="/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/R"+str(i)
        os.chdir(path_to_file)
        temop_mq="qsub -N mutate"+str(i)+" " + path_to_file+"/FOLDX_mutate_round"+str(nround)+"_R"+str(i)+"_pdb"+"_original"+"_dnak.q"
        subprocess.run(temop_mq, shell=True)

    filelist=["/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/R" + str(n)+"/1dkz_Repair_1_0_1_"+str(k)+".pdb" for n in range(1,101) for k in range(0,3)]
    k=True
    while k:
    ...
    for i in range(0,100):
        ...
        for j in chunks[i]:
            for k in range(0,3):
                location="/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/R"+str(i+1)+"/1dkz_Repair_1_0_1_"+str(k)+".pdb"
                destination="/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/1dkz_Repair_1_0_"+str(j)+"_"+str(k)+".pdb"
                command='mv '+location+" "+destination
                subprocess.run(command, shell=True)
                count=count+1  

    path_to_file="/switchlab/group/krinad/dnak/GA_round"+str(nround)        
    os.chdir(path_to_file)
    temop_aq="qsub -N analyse " +"FOLDX_analyse_round"+str(nround)+"_pdb"+"_original"+"_dnak.q"
    subprocess.run(temop_aq, shell=True)   

    os.chdir("/switchlab/group/krinad/dnak/GA_round"+str(nround)+"/analyse")

    while not os.path.exists("Summary_1dkz_Repair_1_0_100_2_AC.fxout"):
        time.sleep(1)
    command="tail -n1 -q Summary_1dkz_Repair_1_0_*.fxout >> Summary_file.txt"
    subprocess.run(command, shell=True)
...
data1=compute_fitness("/switchlab/group/krinad/dnak/GA_round1/analyse/Summary_file.txt",randomlist,1)   
